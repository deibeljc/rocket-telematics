...
FileHandler::FileHandler(FatFileSystem &fs, const char *filename)
  : fatfs(fs), filePath(filename) {}
  ...
  if (batchIndex > 0) {
    ...
    if (dataFile) {
      for (unsigned int i = 0; i < batchIndex; i++) {
        dataFile.println(altitudeBatch[i]);
      }
      dataFile.close();
      clearBatch();
    } else {
...
void FileHandler::sendFileDataOverSerial() {
  ...
  if (dataFile) {
    ...
    Serial.println("millis,temp,acc_x,acc_y,acc_z,gyro_x,gyro_y,gyro_z,altitude,lat,long,state");
